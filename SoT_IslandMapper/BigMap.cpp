...
{
  image_ = CImg<>(file_.c_str()).normalize(0, 255).resize(-100, -100, 1, 3);
...
auto BigMap::isMatch(const TreasureMap& treasure) -> std::pair<char, int>
{
  ...
  for (auto& island : islandBoxes)
  {
    ...
    CImg<unsigned char> clone = treasure.getMask();
    CImg<unsigned char> diff = treasure.getMask();
    diff.fill(0);
...
{
  return image_(x, y, 0, 0) < 128 && image_(x, y, 0, 1) < 128 && image_(x, y, 0, 2) < 128;
}
...
int BigMap::boxIsland(int xPos, int yPos)
{
  ...
  {
    bool black = hasBlack(x, yPos, boxSize, false);
    if (black)
    {
      if (TL[0] == -1)
        TL[0] = x;
