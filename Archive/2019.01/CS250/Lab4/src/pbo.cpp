...
void Pbo::transformToScreen()
{
  ...
  float deg = model->deg * 3.1422 / 180;
  glm::mat4 rotate
  {
    {cos(deg),  0,        sin(deg),   0},
    {0,         1,        0,          0},
...
void Pbo::init(GLsizei w, GLsizei h)
{
  ...
  */
  bool noerror = DPML::parse_obj_mesh("../meshes/teapot.obj", model->verts, model->norms, model->uv, model->indices);
  if (!noerror)
  {
    std::cout << "MESH ERROR\n";
  }
  ...
  // determine all depths
  for (size_t i = 0; i < model->verts.size(); i++)
  {
    int depth = model->verts[i].z;
    depth -= model->min;
    depth *= 128.0f;
    depth /= (model->max - model->min);
    model->depth.push_back(depth);
  }
}
...
void Pbo::drawModel(Model& model)
{
  ...
  {
    float depth = model.norms[model.indices[tri]].z;
    if (model.screen[model.indices[tri]].z < 0)
      continue;

    depth += 1.0f;
    depth /= 2.0f;
    GLubyte normDir = 255 * depth;
    GLubyte zPosition = 255 - model.depth[model.indices[tri]];
    GLubyte average = ((normDir + zPosition) / 2.0f);
    GLubyte colorNorm[3] = 
    {
      255 * ((model.norms[model.indices[tri]].x + 1.0f)/ 2.0f),
      255 * ((model.norms[model.indices[tri]].y + 1.0f)/ 2.0f),
      255 * ((model.norms[model.indices[tri]].z + 1.0f)/ 2.0f)
    };       

    Pbo::Color red = { colorNorm[0],colorNorm[1],colorNorm[2] };
    glm::vec3 thisTri[3];
    ...
    */
    ...
    render_triangle0(thisTri[0], thisTri[1], thisTri[2], red);
  }
...
void Pbo::render()
{
  ...
  clear_color_buffer();
  Pbo::drawModel(*model);
  //render_triangle0(glm::vec3(0, 0, 1), glm::vec3(500, 250, 1), glm::vec3(0, 500, 1), red);
  //render_triangle0(glm::vec3(0, 500, 1), glm::vec3(500, 250, 1), glm::vec3(0, 0, 1), red);
...
void  Pbo::render_triangle0(glm::vec3 const& p0,
  glm::vec3 const& p1,
  glm::vec3 const& p2,
  Pbo::Color color)
{
  ...
  //cull CW triangles
  glm::vec3 crossP = glm::cross(p1 - p0, p2 - p0);
  if (crossP.z < 0.0f)
  {
    // cull it
    return;
  ...
  // find bounds
  glm::vec2 lowerBound;
  lowerBound.x = glm::min(glm::min(p0.x, p1.x), p2.x);
  lowerBound.y = glm::min(glm::min(p0.y, p1.y), p2.y);

  glm::vec2 upperBound;
  upperBound.x = glm::max(glm::max(p0.x, p1.x), p2.x);
  upperBound.y = glm::max(glm::max(p0.y, p1.y), p2.y);

  // fill in pixels
  for (size_t y = 0; lowerBound.y + y < upperBound.y; y++)
  {
    for (size_t x = 0; lowerBound.x + x < upperBound.x; x++)
    {
      bool isValid = true;
      glm::vec3 pixel = { lowerBound.x + x, lowerBound.y + y, 1 };

      // test every edge
      for (size_t edge = 0; edge < 3; edge++)
      {
        // this edge failed - bail out
        if (glm::dot(edges[edge], pixel) < 0)
        {
          isValid = false;
          break;
        }
      ...
      // this passed all edges - fill it in
      if (isValid)
      {
