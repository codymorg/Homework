...
struct OAConfig
{
	...
	struct HeaderBlockInfo
	{
		...
		HeaderBlockInfo(HBLOCK_TYPE type = hbNone, unsigned additional = 0) : type_(type), size_(0), additional_(additional)
		{
			...
			else if (type_ == hbExtended) //user-defined + use counter + alloc #  + flag byte 
				size_ = additional_ + sizeof(unsigned short) + sizeof(unsigned int) + sizeof(char);
...
class ObjectAllocator
{
  ...
  private:
    ...
    OAConfig config_;
    OAStats stats_;
    void allocate_new_page();       // allocates another page of objects
    void put_on_freelist(void *Object); // puts Object onto the free list
    void stampPadding();
    void stampHeader(GenericObject* header = nullptr, int left = 0, bool inuse = 0, const char* label = nullptr);
    void stampExtendedHeader(GenericObject* genericNode, bool inUse);
    unsigned stampAlignment();
    void updateExternalHeader(const char* label, bool inUse, GenericObject* update = nullptr);
    void checkCorruption() const;
    void connectFreePages();
    size_t blockSize = 0;
   //char defaultHeaderValue[2] = {' ','\0'};
