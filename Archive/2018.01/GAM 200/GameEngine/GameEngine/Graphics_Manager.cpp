...
static std::vector<Sprite*> registeredSprites;
...
HRESULT InitDevice()
{
  ...
  if (dxgiFactory2)
  {
    ...
    hr = dxgiFactory2->CreateSwapChainForHwnd(g_pd3dDevice, Get_hWnd(), &sd, nullptr, nullptr, &g_pSwapChain1);
    if (SUCCEEDED(hr))
    {
  ...
  dxgiFactory->MakeWindowAssociation(Get_hWnd(), DXGI_MWA_NO_ALT_ENTER);
  ...
  //g_pImmediateContext->PSSetShaderResources(0, 1, )
    //g_pImmediateContext->PSSetSamplers(0,)
