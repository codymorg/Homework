...
void Scene::ReadAssimpFile(const std::string& path, const Matrix4f& M)
{
  ...
  const aiScene*   aiscene = importer.ReadFile(path.c_str(), aiProcess_Triangulate | aiProcess_GenNormals);
  if (!aiscene)
  {
  ...
  recurseModelNodes(this, aiscene, aiscene->mRootNode, modelTr,path);
}
                       ...
                       const int          level)
{
  ...
  for (unsigned int i = 0; i < node->mNumMeshes; ++i)
  {
    ...
    for (unsigned int t = 0; t < aimesh->mNumVertices; ++t)
    {
      ...
      meshdata->vertices.push_back(VertexData(glm::vec3(aipnt.x, aipnt.y, aipnt.z),
                                              glm::vec3(ainrm.x, ainrm.y, ainrm.z),
                                              glm::vec3(aitex.x, aitex.y, 0),
                                              glm::vec3(aitan.x, aitan.y, aitan.z)));
    }
    ...
    for (unsigned int t = 0; t < aimesh->mNumFaces; ++t)
    {
      ...
      {
        meshdata->triangles.push_back(glm::vec3(aiface->mIndices[0], aiface->mIndices[1], aiface->mIndices[2]));
      }
    }
    
    meshdata->fileName = path;
  ...
  for (unsigned int i = 0; i < node->mNumChildren; ++i)
    recurseModelNodes(scene, aiscene, node->mChildren[i], childTr, path, level + 1);
}
