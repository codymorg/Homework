...
int main(int argc, char** argv)
{
  ...
  WriteHdrImage("ttt" + hdrName, scene->width, scene->height, image, 1);
  for (int y = 0; y < scene->height; y++)
    for (int x = 0; x < scene->width; x++)
      image[y * scene->width + x] = Color(0, 0, 0);
  int passes           = 0;
  int stop             = 512;
  int passPeriteration = 32;
  while (passes < stop)
  {
    scene->tracePath(image, passPeriteration);
    cout << passes << " / " << stop << " iterations\n";
    passes += passPeriteration;
