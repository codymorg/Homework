...
string Client::GetServer(const string& loginServer, const string& username, const string& password)
{
  ...
  std::wstring wide = converter.from_bytes(loginServer);
  http_client httpClient(wide);

  // create user
  cout << "Sending POST for user creation at " << GetTime() << "\n";
  jsonValue json;
  ...
  json[L"avatar"] = jsonValue::string(utility::conversions::to_utf16string(avatar_));
  auto req = httpClient.request(methods::POST, L"/api/v1/users/", json).then([=](http_response res)
  {
    cout << "POST response: ";
    PrintResponse(res);
  });
  ...
  req = httpClient.request(methods::POST, L"/api/v1/login", json).then([&](http_response res)
  {
    cout << "POST response: ";
    PrintResponse(res);
